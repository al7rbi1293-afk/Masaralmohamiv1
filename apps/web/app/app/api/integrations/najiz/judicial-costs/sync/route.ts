...
import { syncNajizJudicialCosts } from '@/lib/integrations/domain/services/najiz-integration.service';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await syncNajizJudicialCosts({
      actor,
    ...
    return NextResponse.json(
      {
        ...
        imported_count: result.costs.length,
        costs: result.costs,
      },
      { status: 200 },
    );
  } catch (error) {
    const response = toIntegrationErrorResponse(error, 'najiz_judicial_costs_sync_failed');
