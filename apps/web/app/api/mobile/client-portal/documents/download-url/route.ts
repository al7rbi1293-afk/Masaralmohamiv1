...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!parsed.success) {
      return NextResponse.json({ error: parsed.error.issues[0]?.message ?? 'تعذر تجهيز رابط التنزيل.' }, { status: 400 });
    }
    ...
    if (versionError || !version) {
      if (versionError) {
        logError('mobile_client_portal_document_download_lookup_failed', { message: versionError.message });
      }
    ...
    if (documentError || !document) {
      if (documentError) {
        logError('mobile_client_portal_document_download_doc_lookup_failed', { message: documentError.message });
      }
      return NextResponse.json({ error: 'المستند غير متاح.' }, { status: 404 });
    }
    ...
    const { data: signed, error: signError } = await db.storage.from('documents').createSignedUrl(parsed.data.storage_path, 300, {
      download: downloadName,
    });

    if (signError || !signed?.signedUrl) {
      logError('mobile_client_portal_document_download_sign_failed', { message: signError?.message ?? 'unknown' });
    ...
    await db.from('audit_logs').insert({
      ...
      user_id: null,
      action: 'mobile_client_portal_document_download_signed',
    ...
    logInfo('mobile_client_portal_document_download_signed', {
      orgId: session.orgId,
    ...
    logError('mobile_client_portal_document_download_unhandled', {
