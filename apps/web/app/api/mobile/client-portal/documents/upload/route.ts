...
import {
  requireClientPortalContext,
} from '@/lib/mobile/client-portal';
import { resolvePortalUploadActorUserId } from '@/lib/client-portal/access';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (matterId) {
      ...
      if (matterError) {
        logError('mobile_client_portal_document_upload_matter_lookup_failed', { message: matterError.message });
        return NextResponse.json({ error: 'تعذر التحقق من القضية المرتبطة.' }, { status: 400 });
      }

      if (!matterRow) {
        return NextResponse.json({ error: 'القضية المحددة غير متاحة لحسابك.' }, { status: 403 });
      }
    ...
    if (!uploaderId) {
      logError('mobile_client_portal_document_upload_actor_missing', { orgId: session.orgId });
      return NextResponse.json({ error: 'تعذر رفع المستند حاليًا. يرجى التواصل مع المكتب.' }, { status: 500 });
    }
    ...
    if (documentError || !document) {
      logError('mobile_client_portal_document_upload_create_failed', { message: documentError?.message ?? 'unknown' });
    ...
    if (uploadError) {
      await db.from('documents').delete().eq('org_id', session.orgId).eq('id', documentId);
      logError('mobile_client_portal_document_upload_storage_failed', { documentId, message: uploadError.message });
      return NextResponse.json({ error: 'تعذر رفع الملف حاليًا.' }, { status: 500 });
    }
    ...
    const { data: version, error: versionError } = await db
      ...
      .select('id, version_no, storage_path, file_name, file_size, mime_type, created_at')
      .single();

    if (versionError || !version) {
      await db.storage.from('documents').remove([storagePath]);
      await db.from('documents').delete().eq('org_id', session.orgId).eq('id', documentId);
      logError('mobile_client_portal_document_upload_version_failed', { documentId, message: versionError?.message ?? 'unknown' });
    ...
    await db.from('audit_logs').insert({
      org_id: session.orgId,
      user_id: null,
      action: 'mobile_client_portal_document_uploaded',
    ...
    logInfo('mobile_client_portal_document_uploaded', {
      orgId: session.orgId,
    ...
    logError('mobile_client_portal_document_upload_unhandled', {
      message: error instanceof Error ? error.message : 'unknown',
    });
    return NextResponse.json({ error: 'تعذر رفع المستند حاليًا.' }, { status: 500 });
  }
}
