...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  try {
    ...
    if (invoiceError) {
      logError('mobile_client_portal_invoice_pdf_invoice_lookup_failed', { message: invoiceError.message });
    ...
    const [{ data: client }, { data: org }, { data: payments }] = await Promise.all([
      db.from('clients').select('name').eq('org_id', session.orgId).eq('id', session.clientId).maybeSingle(),
      db.from('organizations').select('name, logo_url').eq('id', session.orgId).maybeSingle(),
      db.from('payments').select('amount').eq('org_id', session.orgId).eq('invoice_id', invoiceId),
    ]);
    ...
    try {
      ...
      if (error instanceof TimeoutError || error instanceof CircuitOpenError) {
        logWarn('mobile_client_portal_invoice_pdf_transient', { message: error.message });
    ...
    await db.from('audit_logs').insert({
      ...
      user_id: null,
      action: 'mobile_client_portal_invoice_pdf_export',
      entity_type: 'invoice',
      entity_id: invoiceId,
      meta: {
    ...
    logInfo('mobile_client_portal_invoice_pdf_exported', {
      orgId: session.orgId,
    ...
    logError('mobile_client_portal_invoice_pdf_unhandled', {
