...
import { formatDate, sourceLabel, trialRecipientKindActionLabel } from './bulk-messaging-helpers';
import {
  audienceOptions,
  trialRecipientKindOptions,
...
type TrialCampaignSectionProps = {
  ...
  onTrialRecipientKindChange: (value: TrialRecipientKind) => void;
  onPreview: () => void;
  onSend: () => void;
  onBatchSend: () => void;
};
    ...
    <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
            يمكنك الآن فصل الإرسال بين أول مرة ورسائل التذكير، أو إرسال الكل عند الحاجة.
          </p>
        </div>
        <div className="w-full max-w-xs space-y-2 text-sm">
          <span className="font-medium text-slate-700 dark:text-slate-200">نوع الإرسال</span>
          <select
            value={trialRecipientKind}
            onChange={(event) => onTrialRecipientKindChange(event.target.value as TrialRecipientKind)}
            disabled={trialLoading !== null}
            className="h-11 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm outline-none ring-brand-emerald transition focus:border-brand-emerald focus:ring-2 disabled:cursor-not-allowed disabled:opacity-60 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
          >
            {trialRecipientKindOptions.map((option) => (
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Button type="button" onClick={onSend} disabled={trialLoading !== null} className="gap-2">
          ...
          {trialLoading === 'send' ? 'جارٍ الإرسال...' : `إرسال ${trialRecipientKindActionLabel(trialRecipientKind)}`}
        </Button>
        <Button
          type="button"
          variant="secondary"
        ...
        >
          <Send className="h-4 w-4" />
          {trialLoading === 'batch' ? 'جارٍ الإرسال على دفعات...' : 'إرسال المحدد على دفعات'}
        </Button>
      </div>

      {trialRecipientKind !== 'all' ? (
        <div className="rounded-lg border border-brand-emerald/20 bg-brand-emerald/5 px-4 py-3 text-sm text-brand-navy dark:border-brand-emerald/20 dark:bg-brand-emerald/10 dark:text-slate-100">
          المعاينة والإرسال الحاليان مخصصان لـ {trialRecipientKindActionLabel(trialRecipientKind)} فقط.
        </div>
      ) : null}
      ...
      {trialPreview ? (
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard label="المتاح للإرسال الآن" value={trialPreview.stats.selectedRecipients} />
      ...
      {trialPreview && !trialPreview.preview.length ? (
        <div className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200">
          لا يوجد مستلمون ضمن {trialRecipientKindActionLabel(trialPreview.stats.selectedKind)} حالياً.
        </div>
      ) : null}
      ...
      {trialResult ? (
        <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700 dark:border-emerald-900/40 dark:bg-emerald-900/10 dark:text-emerald-300">
          تم إرسال {trialRecipientKindActionLabel(trialResult.stats.selectedKind)}: {trialResult.result.sent} من{' '}
          {trialResult.result.attempted}، وفشل: {trialResult.result.failed}، وتم تحديث حالة{' '}
