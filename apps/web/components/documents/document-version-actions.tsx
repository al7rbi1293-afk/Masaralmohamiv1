...
type DocumentVersionActionsProps = {
  documentId: string;
  latestStoragePath: string | null;
};

export function DocumentVersionActions({ documentId, latestStoragePath }: DocumentVersionActionsProps) {
  ...
  async function downloadLatest() {
    ...
    try {
      const response = await fetch('/app/api/documents/download-url', {
        ...
        body: JSON.stringify({ storage_path: latestStoragePath }),
      });
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-3">
        <Button type="button" variant="outline" size="sm" disabled={busy || !latestStoragePath} onClick={downloadLatest}>
          تنزيل آخر نسخة
        </Button>
      </div>
