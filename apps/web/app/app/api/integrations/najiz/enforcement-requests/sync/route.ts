...
import { syncNajizEnforcementRequests } from '@/lib/integrations/domain/services/najiz-integration.service';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await syncNajizEnforcementRequests({
      actor,
    ...
    return NextResponse.json(
      {
        ...
        imported_count: result.requests.length,
        requests: result.requests,
      },
      { status: 200 },
    );
  } catch (error) {
    const response = toIntegrationErrorResponse(error, 'najiz_enforcement_requests_sync_failed');
