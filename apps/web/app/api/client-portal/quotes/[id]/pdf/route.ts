...
export async function GET(request: NextRequest, context: { params: { id: string } }) {
  ...
  try {
    ...
    if (quoteError) {
      logError('client_portal_quote_pdf_lookup_failed', { message: quoteError.message });
    ...
    try {
      pdfResult = await renderQuotePdfBuffer({
        ...
        qrCode: (quoteRow as any).tax_enabled && (quoteRow.tax_number || org?.tax_number) ? generateZatcaQrCode({
      ...
      if (error instanceof TimeoutError || error instanceof CircuitOpenError) {
        logWarn('client_portal_quote_pdf_transient', { message: error.message });
        return NextResponse.json(
    ...
    await db.from('audit_logs').insert({
      ...
      user_id: null,
      action: 'client_portal_quote_pdf_export',
      entity_type: 'quote',
      entity_id: quoteId,
      meta: {
    ...
    logInfo('client_portal_quote_pdf_exported', {
      orgId: session.orgId,
    ...
    logError('client_portal_quote_pdf_unhandled', {
