...
import { syncNajizDocuments } from '@/lib/integrations/domain/services/najiz-integration.service';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await syncNajizDocuments({
      actor,
    ...
    return NextResponse.json(
      {
        ...
        imported_count: result.documents.length,
        documents: result.documents,
      },
      { status: 200 },
    );
  } catch (error) {
    const response = toIntegrationErrorResponse(error, 'najiz_documents_sync_failed');
