...
export async function GET(request: NextRequest) {
  ...
  const requesterUserIds = Array.from(
    new Set([
      ...subscriptionRows.map((row) => row.requester_user_id).filter((value): value is string => Boolean(value)),
  ...
  const orgIds = Array.from(
    new Set(
      [...subscriptionRows.map((row) => row.org_id), ...paymentRows.map((row) => row.org_id)].filter(
