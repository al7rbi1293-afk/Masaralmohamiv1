...
import { generateClientPortalSessionToken } from '@/lib/client-portal/session';
...
export async function POST(request: NextRequest) {
  ...
  const rate = await checkRateLimit({
    key: `client_portal_mobile:verify_otp:${ip}:${normalizedEmail}`,
    limit: 12,
    windowMs: 10 * 60 * 1000,
  });
  if (!rate.allowed) {
  ...
  return NextResponse.json({
    ...
    session: {
      portal_user_id: String((portalUser as any).id),
      client_id: String((portalUser as any).client_id),
      org_id: String((portalUser as any).org_id),
      email: normalizedEmail,
    },
  });
