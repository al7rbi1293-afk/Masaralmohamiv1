...
export async function POST(request: Request) {
  try {
    ...
    if (templateError) {
      logError('template_upload_requested_failed', { message: templateError.message });
    ...
    if (versionError) {
      logError('template_upload_version_fetch_failed', { message: versionError.message });
      return NextResponse.json({ error: 'تعذر تجهيز رابط الرفع.' }, { status: 400 });
    }

    const nextVersionNo = (latestVersion?.version_no ?? 0) + 1;
    const safeFileName = toSafeFileName(parsed.data.file_name);
    const storagePath = buildStoragePath(orgId, parsed.data.template_id, nextVersionNo, safeFileName);
    ...
    try {
      const result = (await withCircuitBreaker(
        'storage.templates.signed_upload_url',
        { failureThreshold: 3, cooldownMs: 30_000 },
        () =>
          withTimeout(
            service.storage.from('templates').createSignedUploadUrl(storagePath),
            8_000,
      ...
      if (error instanceof TimeoutError || error instanceof CircuitOpenError) {
        logWarn('template_upload_sign_transient', { message: error.message });
        return NextResponse.json(
    ...
    if (signError || !signed) {
      logError('template_upload_sign_failed', { message: signError?.message ?? 'unknown' });
      return NextResponse.json({ error: 'تعذر تجهيز رابط الرفع.' }, { status: 500 });
    }

    logInfo('template_upload_requested', {
    ...
    await logAudit({
      action: 'template.upload_url_issued',
      entityType: 'template',
    ...
    logError('template_upload_requested_failed', {
...
function buildStoragePath(orgId: string, templateId: string, versionNo: number, fileName: string) {
  return `org/${orgId}/tpl/${templateId}/v${versionNo}/${fileName}`;
}
