            ...
            <div className="mt-4 space-y-3 md:hidden">
              {auditEvents.map((row) => (
                <article key={row.id} className="rounded-lg border border-brand-border p-3 dark:border-slate-700">
                  <p className="text-xs text-slate-500 dark:text-slate-400">{formatDateTime(row.created_at)}</p>
                  <p className="mt-1 text-sm font-semibold text-slate-800 dark:text-slate-100">{row.action}</p>
                  <p className="mt-1 text-sm text-slate-700 dark:text-slate-200">{row.entity_type ?? '—'}</p>
