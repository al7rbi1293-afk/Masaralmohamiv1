...
const GENERIC_RESPONSE_MESSAGE = 'إذا كان البريد الإلكتروني مرتبطًا ببوابة العميل، سيتم إرسال رمز التحقق خلال لحظات.';
...
export async function POST(request: NextRequest) {
  ...
  const rate = await checkRateLimit({
    key: `client_portal:request_otp:${ip}:${normalizedEmail}`,
    limit: 5,
    windowMs: 10 * 60 * 1000,
  });
  if (!rate.allowed) {
  ...
  if (!delivery.ok) {
    return NextResponse.json(
      { error: 'تعذر إرسال رمز التحقق عبر البريد الإلكتروني حالياً. حاول بعد قليل.' },
      { status: 502 },
    );
  }
