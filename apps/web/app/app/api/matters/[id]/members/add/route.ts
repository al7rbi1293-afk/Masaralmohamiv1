...
export async function POST(request: NextRequest, context: { params: { id: string } }) {
  ...
  try {
    ...
    if (!targetMembership) {
      return NextResponse.json({ error: 'العضو غير موجود في هذا المكتب.' }, { status: 400 });
    }

    const { error: insertError } = await service.from('matter_members').upsert(
      { matter_id: matterId, user_id: parsed.data.user_id },
    ...
    logInfo('matter_member_added', { matterId, userId: parsed.data.user_id });
...
function toUserMessage(error: unknown) {
  ...
  return 'تعذر إضافة العضو. حاول مرة أخرى.';
}
