 ...
 * PATCH /admin/api/users — suspend/activate/delete users
 */
...
export async function PATCH(request: NextRequest) {
  ...
  if (targetIds.length === 0 || !['suspend', 'activate', 'delete_pending', 'delete'].includes(action)) {
  ...
  if (action === 'delete_pending' || action === 'delete') {
    ...
    if (action === 'delete') {
      // Re-assign restrictive FK references to current admin before deleting users.
