...
export function TaskEmailReminderButton({ taskId }: TaskEmailReminderButtonProps) {
  ...
  async function submit() {
    ...
    try {
      const response = await fetch('/app/api/email/send-task-reminder', {
        method: 'POST',
      ...
      setSuccess('تم إرسال التذكير بالبريد.');
        ...
        >
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                <h3 className="text-lg font-bold text-brand-navy dark:text-slate-100">إرسال تذكير مهمة</h3>
                <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
                  أرسل تذكيرًا يدويًا بالبريد الإلكتروني (بدون أتمتة).
                </p>
              </div>
